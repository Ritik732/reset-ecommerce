    ...
    <div className="flex flex-wrap -m-4">
      ...
      <div key={item.attributes.slug} className="xl:w-1/4 md:w-1/2 p-4">
        ...
        <source src={item.attributes.video.data && item.attributes.video.data.attributes.name} />
          ...
          <Link href={`/product/${item.attributes.slug}`}><button className=" my-2 text-white bg-indigo-500 border-0 py-1 md:py-2 px-4 md:px-4 focus:outline-none hover:bg-indigo-600 rounded text-sm">Buy now</button></Link>
        </div>
      </div>
